<template>
  <div
    ...
    style="height: 260px; overflow-x: hidden; min-width: 820px"
  >
    <div class="item-img">
      <div v-if="booking.Room.Image.length > 0" class="item-img--hasimage">
        <img :src="getLinkImage(booking.Room.Image[0].Path)" alt="anh ks" class="item-img" />
    ...
    <div style="width: 100%">
      <div class="content--column item-info">
        ...
        >
          <span
            >{{ booking.Room.Hotel.Name }} - ({{ booking.Quantity }}x) {{ booking.Room.Name }}</span
          >
          <div>
        ...
        <div class="info-value info__rating">
          {{ renderRating(booking.Room.Hotel.Rating) }}
        </div>
        <div class="content--column">
          <div class="info-value info-time">
            <span>Ngày đặt phòng:</span> <span>{{ formatDate(booking.CreatedAt) }}</span>
          </div>
          <div class="info-value info-time">
            <span>Ngày nhận phòng:</span> <span>{{ formatDate(booking.CheckInDate) }}</span>
          </div>
          <div class="info-value info-time">
  ...
  <ImageGallery :images="booking.Room.Image" :visible="showImagePopup" @close="toggleImagePopup" />
</template>
